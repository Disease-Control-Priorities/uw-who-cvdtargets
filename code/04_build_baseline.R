...
data.in_htn <- unique(
  data.in[, .(location, sex, htncov2, raisedBP)],
  by = c("location", "sex")
)
...
global_raisedBP_pop_2025 <- aim2_base_2025[, sum(pop20 * raisedBP, na.rm = TRUE)]
global_controlled_2025   <- aim2_base_2025[, sum(pop20 * raisedBP * htncov2, na.rm = TRUE)]
...
# ---- Controlled counts MUST include raisedBP
aim2_locsex[, controlled_2025 := pop20 * raisedBP * htncov2_2025]
aim2_locsex[, controlled_2030 := pop20 * raisedBP * htncov2_2030]
...
aim2_loc <- aim2_locsex[, {
  ...
  raisedBP_pop_total <- sum(pop20 * raisedBP, na.rm = TRUE)
