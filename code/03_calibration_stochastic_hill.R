# 03_calibration_indonesia_transparent(2).R
...
# Search: Gaussian coordinate hill climb with occasional restarts.
...
RESTART_EVERY    <- 80          # every Nth iteration, draw a fresh uniform point
...
COMMON_EVALS     <- 40L         # shared IR/CF pair candidates within N_ITER
ADJACENT_PROB   <- 0.20        # occasionally update two adjacent age bands
MIN_PERTURB_SD  <- 0.025       # floor after unsuccessful complete passes
STEP_DECAY      <- 0.70        # multiply SD after a full pass without improvement
...
## ----------------------------------------------------------------------------
## A shared two-parameter fit initializes the age-band vector. Remaining
## evaluations change one band (occasionally two neighbors), score the WHOLE
## projected cohort, and keep only improvements to the combined objective.
calibrate_one_combo <- function(combo_rates, pop_combo, combo_targets, age_match,
      ...
      # Phase 1: common IR/CF pair. The baseline also participates.
      if (granularity == "age_group") {
            n_common <- min(COMMON_EVALS, n_iter)
            for (it in seq_len(n_common)) {
                  pair <- runif(2L, lo, hi)
      ...
      # Phase 2: shuffled complete passes so every age band gets attention.
      order <- integer(0)
      position <- 1L
      pass_improved <- FALSE
      step_sd <- perturb_sd
      ...
      if (best_fit > converge_tol && remaining > 0L) for (it in seq_len(remaining)) {
            if (granularity == "age_group" && position > length(order)) {
                  if (length(order) && !pass_improved)
                        step_sd <- max(MIN_PERTURB_SD, step_sd * STEP_DECAY)
                  order <- sample.int(n_g)
                  position <- 1L
                  pass_improved <- FALSE
            }
            # A full-vector restart occasionally explores a different basin.
            if (restart_every > 0L && it %% restart_every == 0L) {
                  cand <- runif(n_par, lo, hi)
            } else if (granularity == "age_group") {
                  ...
                  cand <- best_par
                  cand[idx] <- reflect_bounds(cand[idx] +
                                                rnorm(length(idx), 0, step_sd),
                                              lo, hi)
            } else {
                  cand <- reflect_bounds(best_par + rnorm(n_par, 0, step_sd),
                                         lo, hi)
            }
            result <- eval_par(cand); n_eval <- n_eval + 1L
            if (result$total < best_total) {
            ...
            if (best_fit <= converge_tol) break
            # Let complete age-band passes finish; do not stop midway through
            # a band merely because other bands failed to improve.
            if (granularity == "combo" && no_improve >= no_improve_limit) break
      }
...
worker_exports <- c(
      ...
      "combo_diag", "multiplier_smoothness", "calibrate_one_combo", "run_combo",
      "GRANULARITY", "SEARCH_HALFWIDTH", "N_ITER", "PERTURB_SD", "RESTART_EVERY",
      "NO_IMPROVE_LIMIT", "COMMON_EVALS", "ADJACENT_PROB",
      "MIN_PERTURB_SD", "STEP_DECAY", "SMOOTH_LAMBDA", "CONVERGE_TOL", "W_DEATHS", "W_PREV", "EPS_REL",
